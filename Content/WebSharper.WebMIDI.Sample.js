...
function connectMIDI(){
  ...
  return new Promise((_2) => {
    _2(new Promise((_3) => {
      _3(globalThis.navigator.requestMIDIAccess().then((d) => {
        statusMessage().Set("MIDI Access Granted!");
        d.inputs.forEach((input) => input.addEventListener("midimessage", handleMIDIMessage));
        return Promise.resolve(null);
      }));
    })["catch"]((d) => {
      const a=unwrapExn(d);
      statusMessage().Set("MIDI Access Denied!");
      const _3=a.message;
      (((_4) => _4("MIDI Connection Error: "+String(_3)))((s) => {
        console.log(s);
      }));
      return Promise.resolve(null);
    }));
...
function handleMIDIMessage(event){
  ...
  (((_1) => _1("MIDI Message: "+String(data)))((s) => {
    console.log(s);
  }));
